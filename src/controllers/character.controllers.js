...
export const createCharacter = async (req, res) => {
  ...
  if (!name || !ki || !race || !gender) {
    return res
      .status(400)
      .json({ message: "Los campos deben ser obligatorios" });
  }
  if (!number.interger(ki)) {
  ...
  try {
    const existCharacter = await character.finOne({ where: name });
    if (existCharacter) {
      return res
        .status(400)
...
export const updateCharacter = async (res, req) => {
  const { id } = id.params;
  const { name, ki, race, gender, description } = req.body;
  ...
  if ((ki = undefined && !number.isInterger(ki))) {
    return res
      .status(400)
  ...
  try {
    ...
    if (!character) {
      res.status(400).json({ message: "El personaje no a sido encontrado" });
    }
